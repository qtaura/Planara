...
export function authorize(opts: { userId?: number; teamId?: number; resource: RBACResource; action: RBACAction }): Promise<{ allowed: boolean; role?: string | null; reason?: string }> {
...
async function logPermission(req: Request, data: { userId: number | null; teamId: number | null; resource: RBACResource; action: RBACAction; outcome: "allowed" | "denied"; reason?: string }) {
  try {
...
export function requirePermission(resource: RBACResource, action: RBACAction) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const teamId = Number((req.params as any).teamId || (req.query as any)?.teamId || (req.body as any)?.teamId || 0) || undefined;
    const result = await authorize({ userId, teamId, resource, action });
    if (!result.allowed) {
      await logPermission(req, { userId: userId ?? null, teamId: teamId ?? null, resource, action, outcome: "denied", reason: result.reason });
      return res.status(result.reason === "unauthorized" ? 401 : 403).json({ error: result.reason || "forbidden" });
    }
    await logPermission(req, { userId: userId ?? null, teamId: teamId ?? null, resource, action, outcome: "allowed" });
    return next();
  };
}
