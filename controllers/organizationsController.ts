...
export async function listMyOrganizations(req: Request, res: Response) {
  try {
    ...
    const memberships = await memRepo.find({ relations: { org: true, team: true, user: true }, where: { user: { id: userId } as any } as any });
...
export async function updateOrganization(req: Request, res: Response) {
  try {
    ...
    const conflict = await orgRepo
      ...
      .where("(o.slug = :slug OR o.nameLower = :nameLower) AND o.id != :id", { slug, nameLower, id: orgId })
      .getOne();
    if (conflict) return res.status(409).json({ error: "organization already exists" });
