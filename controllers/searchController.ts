...
export async function searchTasks(req: Request, res: Response) {
  ...
  try {
    await recordSearchEvent({
      req,
      scope: teamId ? 'team' : 'owner',
      q,
      filters: { type: 'task', status, labels, assignee, projectId, teamId },
...
export async function searchProjects(req: Request, res: Response) {
  ...
  if (q)
    qb.andWhere('(LOWER(project.name) LIKE :q OR LOWER(project.description) LIKE :q)', {
      q: `%${q.toLowerCase()}%`,
    });
  ...
  try {
    await recordSearchEvent({
      ...
      scope: teamId ? 'team' : 'owner',
      q,
      filters: { type: 'project', from, to, teamId },
      resultCount: payload.length,
      started,
    });
  } catch {}
...
export async function searchComments(req: Request, res: Response) {
  ...
  const payload = results.map((c) => ({
    ...
    taskId: (c.task as any)?.id || null,
    projectId: (c.task?.project as any)?.id || null,
    author: (c.author as any)?.username || null,
  }));
  try {
    await recordSearchEvent({
      req,
      scope: teamId ? 'team' : 'owner',
      q,
      filters: { type: 'comment', from, to, projectId, teamId },
      resultCount: payload.length,
      started,
    });
  } catch {}
