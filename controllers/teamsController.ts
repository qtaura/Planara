...
export async function createTeam(req: Request, res: Response) {
  try {
    ...
    const existing = await teamRepo
      ...
      .leftJoin('t.org', 'o')
      .where('o.id = :orgId AND (t.slug = :slug OR t.nameLower = :nameLower)', {
        orgId,
        slug,
        nameLower,
      })
      .getOne();
    if (existing)
      return res.status(409).json({ error: 'team name already exists in organization' });
...
export async function changeRole(req: Request, res: Response) {
  try {
    ...
    const myMembership = memberships.find(
      (m) => (m.team as any)?.id === teamId && (m.user as any)?.id === userId
    );
    ...
    const target = memberships.find(
      (m) => (m.team as any)?.id === teamId && (m.user as any)?.id === targetUserId
    );
    ...
    if (target.role === 'owner' && role !== 'owner') {
      const owners = memberships.filter(
        (m) => (m.team as any)?.id === teamId && m.role === 'owner'
      );
      if (owners.length <= 1)
...
export async function transferTeamOwnership(req: Request, res: Response) {
  try {
    ...
    const newOwnerMem = memberships.find(
      (m) => (m.team as any)?.id === teamId && (m.user as any)?.id === newOwnerUserId
    );
...
export async function leaveTeam(req: Request, res: Response) {
  try {
    ...
    if (myMembership.role === 'owner') {
      ...
      if (owners.length <= 1)
        return res.status(400).json({ error: 'cannot leave as the last owner' });
    }
