...
const ALLOWED_MIME = new Set(["image/png", "image/jpeg", "image/gif", "application/pdf", "text/plain"]);
...
async function getProjectOwnerIdForAttachment(att: Attachment): Promise<number | null> {
  ...
  if (att.task) {
    const t = await taskRepo.findOne({ where: { id: (att.task as any).id }, relations: { project: { owner: true } } });
    return t?.project?.owner ? (t.project.owner as any).id : null;
  }
  if (att.project) {
    const p = await projectRepo.findOne({ where: { id: (att.project as any).id }, relations: { owner: true } });
...
export async function uploadAttachment(req: Request, res: Response) {
  ...
  const { taskId, projectId, fileName, mimeType, size, contentBase64, attachmentId } = req.body || {};
  if (!fileName || !mimeType || typeof size !== "number") return res.status(400).json({ error: "fileName, mimeType, size required" });
  if (size <= 0 || size > MAX_SIZE_BYTES) return res.status(400).json({ error: "file too large" });
  if (!ALLOWED_MIME.has(String(mimeType))) return res.status(400).json({ error: "disallowed file type" });
  ...
  if (taskId) {
    task = await taskRepo.findOne({ where: { id: Number(taskId) }, relations: { project: { owner: true } } });
    if (!task) return res.status(404).json({ error: "task not found" });
  }
  if (!task && projectId) {
    project = await projectRepo.findOne({ where: { id: Number(projectId) }, relations: { owner: true } });
  ...
  if (!teamId) {
    ...
    if (!userId || !ownerId || userId !== ownerId) return res.status(403).json({ error: "forbidden" });
  }

  let att: Attachment | null = null;
  if (attachmentId) {
    att = await attRepo.findOne({ where: { id: Number(attachmentId) }, relations: { task: true, project: true } });
    if (!att) return res.status(404).json({ error: "attachment not found" });
  } else {
    att = attRepo.create({ filename: fileName, mimeType, size, task: task || null, project: project || null, latestVersionNumber: 0, versionCount: 0 });
  ...
  try {
    ...
    if (!scan.clean) {
      await recordAttachmentEvent({ req, eventType: "file_upload_failed", userId: userId ?? null, attachmentId: att.id, mimeType, size, extra: { reason: scan.reason } });
      return res.status(422).json({ error: "virus_detected" });
    }
    fs.writeFileSync(storagePath, buf);
  } catch (e) {
    await recordAttachmentEvent({ req, eventType: "file_upload_failed", userId: userId ?? null, attachmentId: att.id, mimeType, size });
    return res.status(500).json({ error: "upload_failed" });
  }

  const fv = fvRepo.create({ attachment: att, versionNumber, storagePath, mimeType, size });
  await fvRepo.save(fv);
  await attRepo.update({ id: att.id }, { latestVersionNumber: versionNumber, versionCount: (att.versionCount || 0) + 1, size, mimeType });
  const saved = await attRepo.findOne({ where: { id: att.id }, relations: { task: true, project: true } });
  applyRetentionPolicy(saved!);

  try {
    await recordAttachmentEvent({ req, eventType: "file_uploaded", userId: userId ?? null, attachmentId: att.id, mimeType, size, extra: { taskId: task?.id ?? null, projectId: project?.id ?? null, versionNumber } });
  } catch {}
...
export async function getPreview(req: Request, res: Response) {
  ...
  const fv = await fvRepo.findOne({ where: { attachment: { id: att.id }, versionNumber: att.latestVersionNumber } });
  ...
  try {
    ...
    await recordAttachmentEvent({ req, eventType: "file_previewed", userId: userId ?? null, attachmentId: att.id, mimeType: fv.mimeType, size: fv.size });
    stream.pipe(res);
  } catch (e) {
...
export async function deleteAttachment(req: Request, res: Response) {
  ...
  for (const v of versions) {
    try { fs.existsSync(v.storagePath) && fs.unlinkSync(v.storagePath); } catch {}
  }
  await attRepo.remove(att);
  try { await recordAttachmentEvent({ req, eventType: "file_deleted", userId: userId ?? null, attachmentId: att.id, mimeType: att.mimeType, size: att.size }); } catch {}
  res.json({ ok: true });
}
...
export async function listVersions(req: Request, res: Response) {
  const id = Number(req.params.id);
  const fvRepo = AppDataSource.getRepository(FileVersion);
  const versions = await fvRepo.find({ where: { attachment: { id } }, order: { versionNumber: "ASC" } });
  res.json(versions.map(v => ({ id: v.id, versionNumber: v.versionNumber, mimeType: v.mimeType, size: v.size, createdAt: v.createdAt })));
}
...
export async function rollbackVersion(req: Request, res: Response) {
  ...
  const teamId = Number((req.query as any)?.teamId || (req.body as any)?.teamId || 0) || undefined;
  if (!versionNumber || versionNumber < 1) return res.status(400).json({ error: "invalid versionNumber" });
  const attRepo = AppDataSource.getRepository(Attachment);
  const fvRepo = AppDataSource.getRepository(FileVersion);
  const att = await attRepo.findOne({ where: { id }, relations: { task: true, project: true } });
  if (!att) return res.status(404).json({ error: "not found" });
  ...
  const newV = fvRepo.create({ attachment: att, versionNumber: newVersionNumber, storagePath, mimeType: target.mimeType, size: target.size });
  await fvRepo.save(newV);
  await attRepo.update({ id: att.id }, { latestVersionNumber: newVersionNumber, versionCount: (att.versionCount || 0) + 1, size: target.size, mimeType: target.mimeType });

  try { await recordAttachmentEvent({ req, eventType: "file_version_rolled_back", userId: userId ?? null, attachmentId: att.id, mimeType: target.mimeType, size: target.size, extra: { fromVersion: att.latestVersionNumber, toVersion: versionNumber } }); } catch {}
  res.json(sanitize((await attRepo.findOne({ where: { id: att.id }, relations: { task: true, project: true } }))!));
}