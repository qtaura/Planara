    ...
    <div className="flex-1 p-6 overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">Here's what's happening with your projects</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <Card className="p-4">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={Array.from({ length: 10 }).map((_, i) => ({ name: `Week ${i+1}`, value: Math.round(50 + Math.random() * 50) }))}>
              <defs>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="value" stroke="#6366f1" fillOpacity={1} fill="url(#colorPv)" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        {loading ? (
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="h-8" onClick={() => setCreateProjectOpen(true)}>
                <Plus className="h-4 w-4" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.slice(0, 6).map((project, i) => (
              <Card key={project.id} className="p-4">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400">{project.description || 'No description'}</p>
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <TrendingUp className="h-4 w-4 text-slate-400" />
                    <span className="text-sm text-slate-600 dark:text-slate-400">{Math.round(Math.random() * 100)}% complete</span>
                  </div>
                  <Button variant="outline" size="sm" className="h-8" onClick={() => { onSelectProject(String(project.id)); onNavigate('project'); }}>
                    <ArrowUpRight className="h-4 w-4" />
                ...
                <div className="mt-4">
                  {[1,2,3].map((i) => (
                    <div key={i} className="flex items-center justify-between py-2 border-t border-slate-200 dark:border-slate-800/50">
                      <div className="flex items-center gap-2">
